...
export async function createAndStartStory(childId, inputs) {
  ...
  log('Story', 'Triggering generate-story edge function', { storyId: story.id, devMode, allowTemplate })
  if (!devMode) {
    log('Story', 'First illustration batch only — use dev button in reader for more', {
      batchSize: ILLUSTRATION_BATCH_SIZE,
  ...
  fetch(
    `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-story`,
    {
      ...
      body: JSON.stringify({ storyId: story.id, childId, inputs, devMode, allowTemplate }),
    }
  )
    .then(async (res) => {
      ...
      if (!res.ok) {
        ...
        if (body.functionVersion !== '2025-06-17') {
